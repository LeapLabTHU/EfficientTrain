...
bs_list = [512,] * 2 + [256,] * 4 + [128,] * 4
up_freq_list = [1,] * 2 + [2,] * 4 + [4,] * 4
...
for ET_index in range(10):
    ...
    command = f" \
        ...
        --model CSWin_96_24322_base_224 --drop_path {0.5 * args.epoch / 200} --weight_decay 0.10 \
        --use_amp true --opt lamb --opt_eps 1e-6 \
        --model_ema true --model_ema_eval true --model_ema_decay 0.9998 \
        --batch_size {int(bs_list[ET_index] / replay_times_list[ET_index])} --lr 8e-3 --update_freq {up_freq_list[ET_index]} \
